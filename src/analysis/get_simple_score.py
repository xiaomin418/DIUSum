...
def get_detail_sentence_rouge(hyp_file, ref_file, prefix_path, mode):
    ...
    scores['mode'] = mode
    import pickle
    hyp_dir = "/".join(hyp_file.split('/')[:-1]) # /data/xxx/PreEMMS_checkpoints/0328-textonly/hyps
...
def get_mlm_better_avg_rouge2(aiming):
    import pickle
    multimodal_scores = pickle.load(open('{}multimodal-rouge.pickle'.format(tmp_hyp_dir),'rb'))
    textonly_scores = pickle.load(open('{}textonly-rouge.pickle'.format(tmp_hyp_dir),'rb'))
...
def noSelc_performance(pred_g_file, Selc_p_file, noSelc_p_file):
    with open(pred_g_file, 'rb') as f:
        pred_g = pickle.load(f)
        f.close()
    with open(Selc_p_file, 'rb') as f:
        Selc_p = pickle.load(f)
        Selc_p_list = [Selc_p['./images_test/{}.jpg'.format(i+1)][0] for i in range(2000)]
        f.close()
    with open(noSelc_p_file, 'rb') as f:
        noSelc_p = pickle.load(f)
        noSelc_p_list = [noSelc_p['./images_test/{}.jpg'.format(i+1)][0] for i in range(2000)]
        f.close()
    ...
    g1_sp_list = []
    g1_nsp_list = []
    for g, sp, nsp in zip(pred_g, Selc_p_list, noSelc_p_list):
        # import pdb
        # pdb.set_trace
        if g == 0:
            g0_sp_list.append(sp)
            g0_nsp_list.append(nsp)
        else:
            g1_sp_list.append(sp)
            g1_nsp_list.append(nsp)
    # import pdb
    # pdb.set_trace()
    print("g0_sp_list", sum(g0_sp_list)/len(g0_sp_list))
    print("g0_nsp_list", sum(g0_nsp_list)/len(g0_nsp_list))
    print("g1_sp_list", sum(g1_sp_list)/len(g1_sp_list))
    print("g1_nsp_list", sum(g1_nsp_list)/len(g1_nsp_list))
    
    
def selec_result(p1,p2):
    with open(p1, 'rb') as f:
        p1_s = pickle.load(f)
        p1_s_list = [p1_s['./images_test/{}.jpg'.format(i+1)][0] for i in range(2000)]
        print("p1_s_list: ", sum(p1_s_list)/len(p1_s_list))
        f.close()
    with open(p2, 'rb') as f:
        p2_s = pickle.load(f)
        p2_s_list = [p2_s['./images_test/{}.jpg'.format(i+1)][0] for i in range(2000)]
        print("p2_s_list: ", sum(p2_s_list)/len(p2_s_list))
        base_s = sum(p2_s_list)/len(p2_s_list)
        f.close()
    selc_list = []
    for i in range(2000):
        if p1_s_list[i] > p2_s_list[i]:
            selc_list.append(p1_s_list[i])
        else:
            selc_list.append(p2_s_list[i])
    selc_s = sum(selc_list)/len(selc_list)
    print("selc_list: ",sum(selc_list)/len(selc_list))
    print("improve percent: {}".format((selc_s-base_s)*100/base_s))

if __name__=='__main__':
    hyp_file, ref_file = ('/data-xxx/xxx/IGMS_checkpoints/mmss_f4_w05/hyp__16.txt',
                          '/data-xxx/data/xxx/dataset/MMS/corpus/test_title.txt')
    cur_scores = get_detail_sentence_rouge(hyp_file, ref_file, './images_test/', 'textonly')
    # for i in range(2000):
    #     print("i: {}".format(i+1), cur_scores['./images_test/{}.jpg'.format(i+1)][0])

    
    # hyp_file, ref_file = ('/data-xxx/xxx/IGMS_checkpoints/msmo_textonly_cpl/hyp_msmo_img1_19.txt',
    #                       '/data-xxx/data/xxx/dataset/MSMO_Finished/test-10256.txt')
    # cur_scores = get_detail_sentence_rouge(hyp_file, ref_file, './images_test/', 'textonly')
    # for i in range(2000):
    #     print("i: {}".format(i+1), cur_scores['./images_test/{}.jpg'.format(i+1)][0])

    # pred_g_file = "/data-xxx/xxx/IGMS_checkpoints/mmss_f4_LDec/pred_guides_Org_16.pickle"
    # Selc_p_file = "/data-xxx/xxx/IGMS_checkpoints/mmss_f4_LDec/hyp_precisionV2_Org_16.pickle"
    # noSelc_p_file = "/data-xxx/xxx/IGMS_checkpoints/mmss_f4_LDec/hyp_precisionV2_noSelc_16.pickle"
    # noSelc_performance(pred_g_file, Selc_p_file, noSelc_p_file)

    # p1 = "/data-xxx/xxx/IGMS_checkpoints/mmss_f4_LDec/hyp_16_textonly.pickle"
    # p2 = "/data-xxx/xxx/IGMS_checkpoints/mmss_f4_LDec/hyp_precisionV2_Org_16.pickle"
    # selec_result(p1, p2)

    # p1 = "/data-xxx/xxx/IGMS_checkpoints/msmo_textonly_cpl/hyp_msmo_img1_19.pickle"
    # p2 = "/data-xxx/xxx/IGMS_checkpoints/msmo_base_img1_cpl/hyp_msmo_img1_14.pickle"
    # selec_result(p1, p2)
