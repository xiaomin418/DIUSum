...
class BertAbsSum(nn.Module):
    def __init__(self, args, bert_model_path, decoder_config, device, cur_mode):
        super().__init__()
    ...
    def forward(self, img, src_ids, src_mask, key_ids, key_mask, tgt_ids, tgt_mask, img_guide, cur_mode, img_all_use):
        ...
        # img_encoded = img_encoded * img_guide.unsqueeze(1)f.unsqueeze(2)
        if 'img' in cur_mode:
            if self.args.usefuler_assist:
                ...
                if img_all_use==False:
                    ...
                    # img_pad = torch.cat((torch.zeros_like(img_encoded),img_encoded),dim=1)
            ...
            img_encoded = torch.zeros_like(bert_encoded[:,:1,:])
            b = bert_encoded.size(0)
            src_imgs_nonE = torch.ones(b, self.args.img_len).to(device=img.device)
            pred_img_guide = None

        
        src_ids = torch.cat((src_imgs_nonE,src_ids),dim=1).to(dtype=torch.long)
        bert_encoded = torch.cat((img_encoded,bert_encoded),dim=1)
    ...
    def beam_decode(self, img, src_seq, src_mask, beam_size, n_best):
        ...
        def decode_for_imgtxt(img, src_seq, src_mask, beam_size, n_best):
            with torch.no_grad():
                ...
                #-- Encode txt 
                src_enc = self.bert_encoder(src_seq, attention_mask=src_mask, output_all_encoded_layers=False)[0]
                # img_encoded = torch.zeros_like(img_encoded)
                
                if self.args.usefuler_assist:
                    pred_img_guide = self.assist(src_enc[:,:1,:], img_encoded)
                    img_encoded = img_encoded * pred_img_guide[:,:,1:]
                    # img_pad = torch.cat((torch.zeros_like(img_encoded),img_encoded),dim=1)
                    ...
                    pred_max_guide = torch.zeros(8,1,2)
        ...
        if self.fusion_method == 'textonly':
            ...
            batch_hyp, pred_max_guide = decode_for_imgtxt(img, src_seq, src_mask, beam_size, n_best)

        # return batch_hyp, batch_scores
        return batch_hyp, pred_max_guide
